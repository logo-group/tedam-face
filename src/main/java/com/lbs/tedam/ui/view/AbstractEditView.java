...
public abstract class AbstractEditView<T extends AbstractBaseEntity, S extends BaseService<T, Integer>, P extends AbstractEditPresenter<T, S, P, V>, V extends AbstractEditView<T, S, P, V>>
	...
	private TedamVerticalLayout initFolder() {
		TedamVerticalLayout folderHeader = new TedamVerticalLayout();
		folderHeader.setSizeFull();
		folderHeader.setId("folderHeader");

		lblFolder = new TedamLabel();
	...
	private void addSectionToAccordion(String title, int position, Resource icon, TedamCssLayout cssLayout) {
		...
		accordion.addTab(vlayTemp, title, icon, position)
				.setStyleName(com.lbs.tedam.ui.util.Constants.TEDAM_ACCORDION_TAB_CSS);
	}
	...
	public void showDataIntegrityException() {
		TedamNotification.showNotification(getLocaleValue("view.abstractedit.messages.DataIntegrityViolationException"),
				NotifyType.ERROR);
	}
	...
	public TedamLabel getLblFolder() {
		return lblFolder;
	}

	public void setLblFolder(TedamLabel lblFolder) {
