...
@SpringComponent
@ViewScope
public class TestCaseEditPresenter extends AbstractEditPresenter<TestCase, TestCaseService, TestCaseEditPresenter, TestCaseEditView> {
    ...
    @Autowired
    public TestCaseEditPresenter(ViewEventBus viewEventBus, NavigationManager navigationManager, TestCaseService testCaseService, TedamUserService userService,
                                 BeanFactory beanFactory, TestStepDataProvider testStepDataProvider, SnapshotDefinitionService snapshotDefinitionService, LookUpDataProvider lookUpDataProvider,
                                 UploadedFilesDataProvider uploadedFilesDataProvider, TestStepService testStepService, TestCaseTestRunDataProvider testCaseTestRunDataProvider,
                                 PropertyService propertyService) {
        super(viewEventBus, navigationManager, testCaseService, TestCase.class, beanFactory, userService, propertyService);
    ...
    public void testCaseSelectedEvent(TestCaseSelectEvent event) throws LocalizedException {
        ...
        // import files
        try {
            FileUtils.copyDirectory(new File(getPropertyService().getTestcaseFolder(testCaseSource.getId())), new File(getPropertyService().getTestcaseFolder(getItem().getId())));
        } catch (IOException e) {
            e.printStackTrace();
        }
        setHasChanges(true);
    ...
    public void fileAttachedEvent(FileAttachEvent event) throws LocalizedException {
        ...
        for (TestStep testStep : activeTabsGrid.getSelectedItems()) {
            ...
            String expectedFormName = TedamDOMUtils.getExpectedFormName(getPropertyService().getTestcaseFolder(getItem().getId()) + event.getUploadedFileName().getName());
    ...
    public void fileUploadEvent(FileUploadEvent event) throws LocalizedException {
        ...
        uploadedFilesDataProvider.refreshDataProviderByItems(TedamFileUtils.getFiles(getPropertyService().getTestcaseFolder(getItem().getId())));
        getView().organizeUploadedFilesGrid(uploadedFilesDataProvider);
    ...
    @EventBusListenerMethod
    public void testStepTypeParameterPreparedEvent(TestStepTypeParameterPreparedEvent testStepTypeParameterPreparedEvent) {
        testStepTypeParameterPreparedEvent.getTestStep().setParameter(testStepTypeParameterPreparedEvent.getTestStep().getGenerator().generate());
        getView().getActiveTabsGrid().getGridDataProvider().getListDataProvider().refreshItem(testStepTypeParameterPreparedEvent.getTestStep());
    }
    ...
    public void removeTestStepRow() {
        ...
        // TODO:TestSteps are deleted directly from the database. it should be looked at.
        activeTabsGrid.getSelectedItems().forEach(testStep -> activeTabsGrid.getGridDataProvider().removeItem(testStep));
    ...
    public void prepareTestStepTypeWindow(TestStep testStep, ViewMode mode) throws LocalizedException {
        ...
        if (getItem().getId().equals(Integer.valueOf(0))) {
            ...
            return;
        } else if (!TestStepType.getFormOpenTestStepTypeList().contains(testStep.getType()) && !testStep.getType().equals(TestStepType.WAIT) && StringUtils.isEmpty(testStep.getFilename())) {
            // the screen can not be opened while there are no files in the form fill group.
            getView().showFileNotAttachedToTestStepMessage();
            return;
        } else if (TestStepType.getFormOpenTestStepTypeList().contains(testStep.getType()) && !StringUtils.isEmpty(testStep.getFilename())) {
            // the form can not open when there is a file in the open group.
            getView().showFileAttachedToTestStepMessage();
            return;
        } else if (!TestStepType.getFormOpenTestStepTypeList().contains(testStep.getType()) && !testStep.getType().equals(TestStepType.WAIT)
                && Files.notExists(Paths.get(getPropertyService().getTestcaseFolder(testStep.getTestCaseId()) + testStep.getFilename()))) {
            // the screen does not open if the associated file does not exist.
            getView().showFileNotFound();
            return;
        } else if (TestStepType.REPORT.equals(testStep.getType()) && !FilenameUtils.getExtension(testStep.getFilename()).equals(Constants.FILE_EXTENSION_PDF.replace(".", ""))) {
            // no action is taken if the report is not associated with a pdf.
            getView().showPDFFileNotAttachedToTestStepMessage();
            return;
        }
        Map<UIParameter, Object> windowParameters = TedamStatic.getUIParameterMap();
        windowParameters.put(UIParameter.MODE, StringUtils.isEmpty(testStep.getFilename()) == true ? ViewMode.VIEW : ViewMode.EDIT);
    ...
    private void saveDefinitions(TestCase testCase) throws LocalizedException {
        for (TestStep testStep : testCase.getTestSteps()) {
            // the step that was added as the select step of the test will not be the filename.
            if (StringUtils.isEmpty(testStep.getFilename()) || testStep.getType() == null || testStep.getGenerator() == null) {
                continue;
            }
            saveDefinitionsOfTestStepsAndLookUps(testStep);
        }
        for (TestStep testStepLookUp : testCase.getLookUps()) {
            if (StringUtils.isEmpty(testStepLookUp.getFilename()) || testStepLookUp.getType() == null || testStepLookUp.getGenerator() == null) {
                continue;
            }
    ...
    public void onTestStepTypeChange(TestStep testStep, TestStepType newTestStepType, AbstractDataProvider<TestStep> abstractDataProvider) {
    ...
    public void changeTestStepType(TestStep testStep, TestStepType newTestStepType, AbstractDataProvider<TestStep> abstractDataProvider) {
    ...
    public void resetTestStep(TestStep testStep, AbstractDataProvider<TestStep> abstractDataProvider) throws LocalizedException {
        testStep.setGenerator(null);
        ...
        testStep.setParameter(null);
        // Since this is a problem, it is closed with cancel in this way, a wide can be viewed.
        // can be viewed.
