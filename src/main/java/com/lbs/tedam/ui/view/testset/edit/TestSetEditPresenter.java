...
@SpringComponent
@ViewScope
public class TestSetEditPresenter
		extends AbstractEditPresenter<TestSet, TestSetService, TestSetEditPresenter, TestSetEditView> {
	...
	@Autowired
	public TestSetEditPresenter(ViewEventBus viewEventBus, NavigationManager navigationManager,
			TestSetService testSetService, TedamUserService userService, BeanFactory beanFactory,
			TestCaseDataProvider testCaseDataProvider, TestCaseTestRunDataProvider testCaseTestRunDataProvider,
			PropertyService propertyService, TedamFolderService folderService) {
		super(viewEventBus, navigationManager, testSetService, TestSet.class, beanFactory, userService,
				propertyService);
	...
	private void setTestCasePosition(TestSet testSet) {
		for (ListIterator<TestSetTestCase> listIterator = testSet.getTestSetTestCases().listIterator(); listIterator
				.hasNext();) {
	...
	protected void getTitleForHeader() {
		if (getItem().getName() != null) {
			try {
				String folderName;
				folderName = folderService.getById(getItem().getTestSetFolderId()).getName();
				getView().setTitle(folderName + " / " + getView().getTitle() + ": " + getItem().getName());
